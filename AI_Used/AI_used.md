...
- `tls_fingerprint/ja3.py` vs. [salesforce/ja3](https://github.com/salesforce/ja3): same list of GREASE values, same JA3 formula and field order, same extension codes used for curves/point-formats. Manually adapted, not written independently.
- `tls_fingerprint/tls_parser.py` vs. [salesforce/ja3](https://github.com/salesforce/ja3): parses the ClientHello in the same order (version, session id, ciphers, compression, extensions). Rewritten by hand to drop the `dpkt` dependency the reference uses.
